...
class VggModel(nn.Module):
    def __init__(self, n_class, fine_tune, pretrained=True):
        ...
        net = models.vgg11_bn(progress=True, pretrained=pretrained)
        # net.classifier = nn.Sequential()
        # net.avgpool = nn.Identity()
        set_parameter_requires_grad(net, fine_tune)
        self.features = nn.Sequential(*list(net.children())[:-2])
        
        #self.avgpool = nn.AdaptiveAvgPool2d((7, 7))
        self.classifier = nn.Sequential(    #Define your own classification layer
                nn.Linear(512, 512),  #512 * 7 * 7 can't be changed. The second parameter is determined by VGG16 network. The number of neurons can be adjusted
                nn.BatchNorm1d(512),
                nn.ReLU(True),
    ...
    with torch.no_grad():
        ...
        if current_loss > last_loss:
            ...
            if trigger_times >= patience:
                ...
                torch.save(checkpoint,'BestModel.pth.tar')
                break
        else:
            trigger_times = 0
...
plt.plot(loss_val, 'b', label='Val loss')
plt.legend()
plt.show()
...
checkpoint = torch.load("BestModel.pth.tar")
