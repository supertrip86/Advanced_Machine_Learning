...
input_size = 32 * 32 * 3
hidden_size = [50, 75, 125, 250]
num_classes = 10

blr, blrd, brg, bni, bbs, bva = 0.0001, 0.95, 0.25, 1000, 200, 0
bhs = 0

for i in hidden_size:
    temp_network = TwoLayerNet(input_size, i, num_classes)

    stats = temp_network.train(X_train, y_train, X_val, y_val,
                          num_iters=bni, batch_size=bbs,
                          learning_rate=blr, learning_rate_decay=blrd,
                          reg=brg, verbose=True)

    # Predict on the validation set
    val_acc = (temp_network.predict(X_val) == y_val).mean()

    if val_acc > bva:
        bhs = i

print("best hidden size is:", bhs)

grid = {'learning_rate': [1e-4, 2e-4, 1e-3,2e-3],
        'learning_rate_decay': [0.99, 0.90, 0.88,0.75],
        'reg': [0.2, 0.3, 0.5, 0.15],
        'num_iters': [600, 500, 800, 600],
        'batch_size': [128, 256, 512, 1024]}

network = TwoLayerNet(input_size, bhs, num_classes)

print("started")

for x in range(20):

    lr = random.sample(grid["learning_rate"], 1)[0]
    lrd = random.sample(grid["learning_rate_decay"], 1)[0]
    rg = random.sample(grid["reg"], 1)[0]
    ni = random.sample(grid["num_iters"], 1)[0]
    bs = random.sample(grid["batch_size"], 1)[0]
    print("try: ", x, lr, lrd, rg, ni, bs)
...
print("best ", bva, blr, blrd, brg, bni, bbs, bva)

stats = network.train(X_train, y_train, X_val, y_val,
            num_iters=2000, batch_size=bbs,
            learning_rate=blr, learning_rate_decay=blrd,
            reg=brg, verbose=True)
...
test_acc = (network.predict(X_test) == y_test).mean()
